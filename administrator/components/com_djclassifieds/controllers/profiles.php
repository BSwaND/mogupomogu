...
class DJClassifiedsControllerProfiles extends JControllerAdmin
{
	...
	function clearInactiveData(){
		...
		."FROM #__djcf_profiles p "
		."LEFT JOIN #__users u ON p.user_id=u.id "
		."WHERE u.id IS NULL";
		$db->setQuery($query);
		$inactive_uids = $db->loadColumn();	
		
		$count = 0;
		foreach($inactive_uids as $uid){				
			$query = "SELECT * FROM #__djcf_images WHERE item_id = ".$uid." AND type='profile'";
			$db->setQuery($query);
			$user_images =$db->loadObjectList('id');				
		...
		if($count){
			$app->enqueueMessage(JText::sprintf('COM_DJCLASSIFIEDS_INACTIVE_PROFILES_CLEARED', $count));
		}else{
			$app->enqueueMessage(JText::_('COM_DJCLASSIFIEDS_NO_INACTIVE_PROFILE_DATA_FOUND'));
		}
	...
	public function batch(){
		...
		$redirect = 'index.php?option=com_djclassifieds&view=profiles';
		...
		$verified = $app->input->getCmd('batch_verified', '');
		$ug_id = $app->input->getCmd('batch_ugid', '');
		$email_body = $app->input->getStr('batch_email', '');
		$items_updated = '0';

	    if($id_arr && ($verified != '' || $ug_id != '')){
	        $ids = implode(',', $id_arr);
			
			if($verified != ''){
				$query = "UPDATE #__djcf_profiles SET verified=".$verified." WHERE user_id IN (".$ids.")";
				$db->setQuery($query);
				$db->execute();
			}
			...
			if($ug_id != ''){
				foreach($id_arr as $uid){
					//JUserHelper::setUserGroups($uid, array($ug_id));
					JUserHelper::addUserToGroup($uid, $ug_id);
		...
		$app->enqueueMessage(JText::sprintf('COM_DJCLASSIFIEDS_BATCH_PROFILES_UPDATED', $items_updated));

	    if($id_arr && $email_body){
			$fromname = $config->get('sitename');
			$mailfrom = $config->get('mailfrom', '');
			$fromname = $config->get('fromname', '');
			$replyto = $config->get('replyto', '');
			$replytoname = $config->get('replytoname', '');
			$subject = JText::sprintf('COM_DJCLASSIFIEDS_BATCH_PROFILES_EMAIL_SUBJECT', $config->get('sitename', ''));
			$message = $email_body;
			...
			$app->enqueueMessage(JText::sprintf('COM_DJCLASSIFIEDS_BATCH_PROFILES_EMAILS_SENT', count($id_arr)));
		}
