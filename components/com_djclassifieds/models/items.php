...
class DjclassifiedsModelItems extends JModelLegacy{	
	...
	function getItems($catlist=''){
		...
		if(($app->input->getStr('se_postcode') || $app->input->getStr('se_address')) && !$app->input->getInt('se_radius')){
				...
				if(JRequest::getVar('se','0','','string')!='0'){			
					if(JRequest::getVar('search',JText::_('COM_DJCLASSIFIEDS_SEARCH'),'','string')!=JText::_('COM_DJCLASSIFIEDS_SEARCH') && JRequest::getVar('search','','','string')){
						...
						if($par->get('search_type_phrase',0)==1){
							...
							foreach($se_words as $se_w){
								if(strlen($se_w)>2){
									$search .= " AND (CONCAT(i.name,i.intro_desc,i.description,IFNULL(i.contact,'')) LIKE ".$db->Quote('%'.$se_w.'%')." OR c.name LIKE ".$db->Quote('%'.$se_w.'%')." OR r.name LIKE ".$db->Quote('%'.$se_w.'%')." OR i.id=".$db->Quote($se_w)." OR ".$u_name_search." LIKE ".$db->Quote($se_w)." ) ";
								}
							}	
						}else{
							...
							$search = " AND (CONCAT(i.name,i.intro_desc,i.description,IFNULL(i.contact,'')) LIKE ".$search_word." OR c.name LIKE ".$search_word." OR r.name LIKE ".$search_word." OR i.id=".$search_word2." OR ".$u_name_search."=".$search_word2." ) ";
						}
					}
					...
					if($radius){
						...
						if($se_geoloc){
							...
							// $se_lat = JRequest::getVar('se_lat','');
							// $se_lng = JRequest::getVar('se_lng','');
							// if($se_lat && $se_lng){
							// 	$se_address_coord = array('lat' => $se_lat, 'lng' => $se_lng);
							// }else{
			...
			if($order=="title"){
				...
				$version = (empty($version->version)) ? '1.0' : $version->version;
				...
				if (version_compare($version, '1.3.2', '<')) {
					...
					$rev_lj = "LEFT JOIN #__djrevs_objects rev ON i.id = rev.entry_id AND rev.item_type='com_djclassifieds.item' ";
				}	
			}
			...
			$dispatcher->trigger('onDJClassifiedsListingQuery', array(&$query, &$limitstart, &$limit));
			...
			if(count($items)){
				...
				$query = "SELECT fv.item_id, fv.field_id, fv.value, fv.value_date, fv.value_date_to, f.label, f.type, f.date_format FROM #__djcf_fields_values fv,#__djcf_fields f "
	...
	function getCountItems($catlist=''){
			...
			$query = "SELECT count(i.id) FROM (SELECT i.id ".$search_radius_v." FROM ".$search_fields." #__djcf_items i "
				...
				$dispatcher->trigger('onDJClassifiedsListingQueryCount', array(&$query));
	...
	function getSearchFields($cat_id, $se_ef){
		$search_fields = '';
			...
			$session->set('last_parent_cid',$parent_cid);
					...
					if($f->type=='date'){						
									...
									$search_fields .= " (SELECT * FROM #__djcf_fields_values f WHERE f.field_id=".$f->id." ";
										if($f_v1!=''){
											if($f_v1!='0000-00-00 00:00:00' && $f->date_format){
												$f_v1 = date_format(date_create_from_format($f->date_format, $f_v1), 'Y-m-d');
											}
											$search_fields .= " AND f.value_date >= '".$f_v1."'";
										}
										if($f_v2!=''){
											if($f_v2!='0000-00-00 00:00:00' && $f->date_format){
												$f_v2 = date_format(date_create_from_format($f->date_format, $f_v2), 'Y-m-d');
											}
						...
						if($f->search_type=='select_min_max' || $f->search_type=='inputbox_min_max'){
							...
							if(isset($_GET['se_'.$f->id])){
								...
								if($v_chec){
									$sf_count ++;
									$search_fields .= " (SELECT * FROM #__djcf_fields_values f WHERE f.field_id=".$f->id." AND ( ";
									$v_c_arr = array();
						...
						elseif($f->search_type=='select'){
							if(isset($_GET['se_'.$f->id])){							
								...
								if($f_v1!=''){
									...
									$search_fields .= " (SELECT * FROM #__djcf_fields_values f WHERE f.field_id=".$f->id." AND ".$val_collate.") UNION ";
									$session->set('se_'.$f->id,$f_v1);								
								}else{
									$session->set('se_'.$f->id,'');	
								}
	...
	function getCustomFields(){
		$db= JFactory::getDBO();
		$query = "SELECT * FROM #__djcf_fields WHERE published=1 AND (in_table>0 OR in_blog=1) ORDER BY ordering";
		$db->setQuery($query);
