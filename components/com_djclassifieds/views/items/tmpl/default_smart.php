					...
					<div class="dj-items_order_by_in">
						<div class="dj-items_order_by_label"><?php echo JText::_('COM_DJCLASSIFIEDS_SORT_BY')?></div>															
							...
							if (in_array("title", $sort_fields) || $sort_fields_c==0){ ?>
								...
								<a class="<?php echo $sort_class; ?>" href="index.php?option=com_djclassifieds&view=items&cid=<?php echo $main_slug; ?>&order=title&ord_t=<?php echo $ord_t.'&Itemid='.$Itemid;?><?php echo $se_link;if($uid){ echo '&uid='.$uid; }?>">
							...
							<?php if (in_array("cat", $sort_fields) || $sort_fields_c==0){ ?>
								...
								<a class="<?php echo $sort_class; ?>" href="index.php?option=com_djclassifieds&view=items&cid=<?php echo $main_slug; ?>&order=cat&ord_t=<?php echo $ord_t.'&Itemid='.$Itemid;?><?php echo $se_link;if($uid){ echo '&uid='.$uid; }?>">
							...
							<?php if (in_array("loc", $sort_fields) || $sort_fields_c==0){ ?>
								...
								<a class="<?php echo $sort_class; ?>" href="index.php?option=com_djclassifieds&view=items&cid=<?php echo $main_slug; ?>&order=loc&ord_t=<?php echo $ord_t.'&Itemid='.$Itemid;?><?php echo $se_link;if($uid){ echo '&uid='.$uid; }?>">
							...
							<?php if (in_array("price", $sort_fields) || $sort_fields_c==0){ ?>
								...
								<a class="<?php echo $sort_class; ?>" href="index.php?option=com_djclassifieds&view=items&cid=<?php echo $main_slug; ?>&order=price&ord_t=<?php echo $ord_t.'&Itemid='.$Itemid;?><?php echo $se_link;if($uid){ echo '&uid='.$uid; }?>">
							...
							<?php if (in_array("date_a", $sort_fields) || $sort_fields_c==0){ ?>
								...
								<a class="<?php echo $sort_class; ?>" href="index.php?option=com_djclassifieds&view=items&cid=<?php echo $main_slug; ?>&order=date_a&ord_t=<?php echo $ord_t.'&Itemid='.$Itemid;?><?php echo $se_link;if($uid){ echo '&uid='.$uid; }?>">
									<?php echo JText::_('COM_DJCLASSIFIEDS_DATE_ADDED'); ?>
								</a>
							<?php } ?>
							<?php if (in_array("date_e", $sort_fields) || $sort_fields_c==0){ ?>
								<span class="item_orderby_separator"></span>		
								<?php if($order=="date_e"){ $sort_class = ($ord_t=="desc")? 'active active_asc' : 'active active_desc'; }else{$sort_class='';}?>
								<a class="<?php echo $sort_class; ?>" href="index.php?option=com_djclassifieds&view=items&cid=<?php echo $main_slug; ?>&order=date_e&ord_t=<?php echo $ord_t.'&Itemid='.$Itemid;?><?php echo $se_link;if($uid){ echo '&uid='.$uid; }?>">
							...
							<?php if (in_array("display", $sort_fields) || $sort_fields_c==0){ ?>
								...
								<a class="<?php echo $sort_class; ?>" href="index.php?option=com_djclassifieds&view=items&cid=<?php echo $main_slug; ?>&order=display&ord_t=<?php echo $ord_t.'&Itemid='.$Itemid;?><?php echo $se_link;if($uid){ echo '&uid='.$uid; }?>">
							...
							<?php if (in_array("distance", $sort_fields) || $sort_fields_c==0){ ?>
								...
								<a class="<?php echo $sort_class; ?>" href="index.php?option=com_djclassifieds&view=items&cid=<?php echo $main_slug; ?>&order=distance&ord_t=<?php echo $ord_t.'&Itemid='.$Itemid;?><?php echo $se_link;if($uid){ echo '&uid='.$uid; }?>">
							...
							<?php if (file_exists( JPATH_ROOT . '/plugins/djclassifieds/djreviews/djreviews.php' ) ) { ?>						
								...
								<a class="<?php echo $sort_class; ?>" href="index.php?option=com_djclassifieds&view=items&cid=<?php echo $main_slug; ?>&order=reviews&ord_t=<?php echo $ord_t.'&Itemid='.$Itemid;?><?php echo $se_link;if($uid){ echo '&uid='.$uid; }?>">
										...
										<div class="item_img_box" >
											<div class="item_img_box_in">					
												<a href="<?php echo DJClassifiedsSEO::getItemRoute($i->id.':'.$i->alias,$i->cat_id.':'.$i->c_alias,$i->region_id.':'.$i->r_name,$this->main_cat,$i->extra_cats).$anch; ?>">
													<?php 
													if(count($i->images)){ 
														echo '<img src="'.JURI::base(true).$i->images[0]->thumb_smart_table.'"';
																		...
																		if($i->latitude && $i->longitude){
																			if(isset($_COOKIE["djcf_latlon"])) {
																				...
																				echo ' <a class="show_on_map" target="_blank" href="http://maps.google.com/maps?saddr='.str_ireplace('_', ',', $_COOKIE["djcf_latlon"]).'&daddr='.$daddr.'" >';
																					...
																					echo '</span>';
																				echo '</a>';
																			}elseif((JRequest::getVar('se_address','') || JRequest::getVar('se_postcode','')) && isset($i->distance)) {
																				echo '<span title="'.(JRequest::getVar('se_address','') ? JRequest::getVar('se_address','') : JRequest::getVar('se_postcode','')).'">';
													...
													for($cf_i=0;$cf_i<count($cf_active_all);$cf_i++){
														if(isset($i->fields[$cf_active_all[$cf_i]])){
															...
															echo '<div class="item_cf_box">';
																...
																if($i->fields[$cf_active_all[$cf_i]]==''){
																	echo '---'; 
																}else{
																	if($this->custom_fields[$cf_active_all[$cf_i]]->type=='checkbox'){
																		$val_arr = explode(';', $i->fields[$cf_active_all[$cf_i]]);
																		$val_arr = array_filter($val_arr);
																		if($par->get('cf_values_to_labels','0')){
																			$val_arr = array_map(function($v){ 
																				return JText::_('COM_DJCLASSIFIEDS_'.str_ireplace(' ', '_', strtoupper($v)));
																			}, $val_arr);
																		}                                             
																		echo implode(', ', $val_arr);
																	}elseif($this->custom_fields[$cf_active_all[$cf_i]]->type=='link'){
																	...
																	}else if($this->custom_fields[$cf_active_all[$cf_i]]->type=='date'){		
																		$f = $this->custom_fields[$cf_active_all[$cf_i]];					
																		if($i->fields[$cf_active_all[$cf_i]]=='0000-00-00'){echo '---'; }
																		else{
																			if(!$f->date_format){$f->date_format = 'Y-m-d';}
																			echo DJClassifiedsTheme::formatDate(strtotime($i->fields[$cf_active_all[$cf_i]]),'','',$f->date_format);
																		}
																	}else{
																		if($par->get('cf_values_to_labels','0') && !in_array($this->custom_fields[$cf_active_all[$cf_i]]->type, array('inputbox', 'textarea', 'date_from_to'))){
