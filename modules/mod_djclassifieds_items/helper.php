...
class modDjClassifiedsItems{
	public static function getItems($params,$cfpar=null){
		...
		if($params->get('items_ord')==1){
			...
			$prom_special_lj = "LEFT JOIN (SELECT item_id, max(date_exp) date_exp FROM #__djcf_items_promotions WHERE prom_id=5 GROUP BY item_id) ip ON i.id=ip.item_id ";
		...
		if($user_groups){
			...
			$user_groups = ' AND i.user_id IN ('.$users_in_groups.')';
		}
		...
		if($params->get('fallow_region','0')==1 && JRequest::getVar('option','')=='com_djclassifieds'){						
			...
			}else if($reg_id = JRequest::getInt('rid',0)){
				$regs = $djcfreglib->getSubReg($reg_id);
				$reglist = $reg_id;
				foreach($regs as $r){
					$reglist .= ','. $r->id;
				}
		...
		$query = "SELECT i.*,c.id as c_id, c.name as c_name,c.alias as c_alias,c.icon_url as c_icon_url, r.id as r_id, r.name as r_name "
				...
				.$promoted.$item_ids.$users_ids.$user_groups.$fallow_cat.$cat_list.$fallow_region.$region_list.$follow_user.$types_ids.$only_img.$source.$access_view.$current_ad.$adult_restriction.$auctions_restriction
