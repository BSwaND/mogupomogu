...
class DjclassifiedsModelUserItems extends JModelLegacy{	
	...
	function getItems(){
		...
		$db			= JFactory::getDBO();
			...
			$query = "SELECT i.*,c.id as c_id, c.name AS c_name, c.alias AS c_alias,r.id as r_id, r.name as r_name, i.date_start <= '".$date_now."' AND i.date_exp >= '".$date_now."' AND i.blocked=0 AND i.published!=2 AS s_active FROM #__djcf_items i "
			...
			$dispatcher->trigger('onDJClassifiedsUseritemsQuery', array(&$query, &$limitstart, &$limit));
	...
	function getCountItems(){
			...
			$dispatcher = JDispatcher::getInstance();
			$dispatcher->trigger('onDJClassifiedsUseritemsQueryCount', array(&$query));
