...
class DjClassifiedsModelFieldsXref extends JModelList{
	...
	function getFields(){
		if(empty($this->_fieldsxref)) {
			...
			$db= JFactory::getDBO();
			$query = "SELECT f.id, f.name, f.label, f.type, fx.id as xref, fx.ordering, IF((fx.id IS NULL), 0, 1) as active FROM #__djcf_fields f "
			."LEFT JOIN (SELECT * FROM #__djcf_fields_xref WHERE cat_id=".$cat_id.") as fx "
			."ON fx.field_id=f.id "
			."WHERE f.published=1 AND f.source=0 ORDER BY active DESC, ".$orderCol." ".$orderDirn." ";
