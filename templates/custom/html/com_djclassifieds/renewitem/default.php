...
<div id="dj-classifieds" class="clearfix djcftheme-<?php echo $par->get('theme', 'default'); ?>">
	...
	<div class="dj-additem dj-renewitem clearfix">
		<form action="<?php echo JRoute::_('index.php'); ?>" method="post" class="form-validate" name="djForm" id="djForm"
		      enctype="multipart/form-data">
			<div class="additem_djform">

				<div
						class="title_top"><?php echo JText::_('COM_DJCLASSIFIEDS_RENEW_ADVERT') . '&nbsp;-&nbsp;' . $this->item->name; ?></div>
				<div class="additem_djform_in">
					<center><img src='<?php echo JURI::base(true) ?>/components/com_djclassifieds/assets/images/long_loader.gif'
					             alt='LOADING' style='display: none;' id='upload_loading'/>
						<div id="alercik"></div>
					</center>
					<?php
							...
							<div class="djform_row">
								<label class="label" for="type_id" id="type_id-lbl">
									<?php echo JText::_('COM_DJCLASSIFIEDS_TYPE');
										if ($par->get('types_required', '0')) {
											echo ' * ';
										} ?>
								</label>
								<div class="djform_field">
									<select autocomplete="off" name="type_id" id="type_id"
									        class="inputbox<?php if ($par->get('types_required', '0')) {
										        echo ' required';
									        } ?>">
						...
						if ($par->get('durations_list', '') && count($this->days)) {
							...
							<div class="djform_row">
								<?php if ($par->get('show_tooltips_newad', '0')) { ?>
									<label class="label Tips1" for="exp_days" id="exp_days-lbl"
									       title="<?php echo JTEXT::_('COM_DJCLASSIFIEDS_EXPIRE_AFTER_TOOLTIP') ?>">
										<?php echo JText::_('COM_DJCLASSIFIEDS_EXPIRE_AFTER'); ?>
										<img src="<?php echo JURI::base(true) ?>/components/com_djclassifieds/assets/images/tip.png"
										     alt="?"/>
									</label>
								<?php } else { ?>
								...
								<select id="exp_days" name="exp_days">
									<?php
										foreach ($this->days as $day) {
											...
											if ($day->days == $this->item->exp_days) {
												echo ' SELECTED ';
											}
											echo '>';
