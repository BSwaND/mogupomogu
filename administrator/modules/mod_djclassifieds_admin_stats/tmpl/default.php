...
<?php if($params->get('ads_doughnut','1') || $params->get('users_doughnut','1')){ ?>

<section class="row-fluid">

<?php if($params->get('ads_doughnut','1')){ ?>
  
  <div class="djcf_admin_stats_box djcf_admin_stats_doughnut" id="doughnut1">
      <span class="chart-header"><?php echo JText::_('MOD_DJCLASSIFIEDS_ADMIN_STATS_HEADER_ADS') ?></span>

      <canvas id="doughnut1_canvas" height="200"></canvas>
      
      <script>

          var arr = <?php echo json_encode(modDjClassifiedsStats::getDataDoughnut1()); ?>;
          
          var values = [], colors = [], labels = [];
          arr.forEach(function(el, i) {
            values.push(arr[i]['value']);
            colors.push(arr[i]['color']);
          ...
          var chart1 = new Chart('doughnut1_canvas', {
            type: 'doughnut',
            data: data,
            options: {
              ...
              hover: {
                onHover: function(e, el) {
                  jQuery('#doughnut1_canvas').css('cursor', el[0] ? 'pointer' : 'default');
                }
              }
            }
          });
        
          jQuery('#doughnut1_canvas').click( 
              function(evt){
                  var activePoints = chart1.getElementsAtEvent(evt);
                  if(activePoints.length){
                    window.open(arr[activePoints[0]._index].link, '_blank');
                  }
              }
          );
  ...
  <?php if($params->get('users_doughnut','1')){ ?>

  <div class="djcf_admin_stats_box djcf_admin_stats_doughnut" id="doughnut2">
      <span class="chart-header"><?php echo JText::_('MOD_DJCLASSIFIEDS_ADMIN_STATS_HEADER_USERS') ?></span>
      <canvas id="doughnut2_canvas" height="200"></canvas>
      
      <script>

          var arr2 = <?php echo json_encode(modDjClassifiedsStats::getDataDoughnut2()); ?>;
          
          var values = [], colors = [], labels = [];
          arr2.forEach(function(el, i) {
            values.push(arr2[i]['value']);
            colors.push(arr2[i]['color']);
            labels.push(arr2[i]['label']);
          });
          ...
          var chart2 = new Chart('doughnut2_canvas', {
            type: 'doughnut',
            data: data,
            options: {
              maintainAspectRatio: false,
              ...
              hover: {
                onHover: function(e, el) {
                  jQuery('#doughnut2_canvas').css('cursor', el[0] ? 'pointer' : 'default');
                }
              }
            }
          });
        
          jQuery('#doughnut2_canvas').click( 
              function(evt){
                  var activePoints = chart2.getElementsAtEvent(evt);
                  if(activePoints.length){
                    window.open(arr2[activePoints[0]._index].link, '_blank');
                  }
              }
          );
...
<?php } ?>
  ...
  <section class="row-fluid">
      ...
      <?php if($app->input->get('option') == 'com_djclassifieds' && $app->input->get('view') == 'cpanel'){ ?>
  ...
  <section class="row-fluid row-boxes">
