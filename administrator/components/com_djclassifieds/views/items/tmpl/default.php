...
<form action="index.php?option=com_djclassifieds&view=items" method="post" name="adminForm" id="adminForm">
	...
	<div id="j-main-container" class="span10">
		<div id="filter-bar" class="btn-toolbar">
			...
			<div class="btn-group pull-right hidden-phone">
				<select name="filter_blocked" class="inputbox" onchange="this.form.submit()">
					<option value=""><?php echo JText::_('COM_DJCLASSIFIEDS_SELECT_BLOCKED');?></option>
					<?php echo JHtml::_('select.options', array(JHtml::_('select.option', '1', 'COM_DJCLASSIFIEDS_BLOCKED'),JHtml::_('select.option', '0', 'COM_DJCLASSIFIEDS_NOT_BLOCKED')), 'value', 'text', $this->state->get('filter.blocked'), true);?>
				</select>
			</div>				
			<div class="btn-group pull-right hidden-phone">
				<select name="filter_active" class="inputbox" onchange="this.form.submit()">
					...
					<?php echo JHtml::_('select.options', array(JHtml::_('select.option', '1', 'COM_DJCLASSIFIEDS_ACTIVE'),JHtml::_('select.option', '0', 'COM_DJCLASSIFIEDS_EXPIRED')), 'value', 'text', $this->state->get('filter.active'), true);?>
				</select>
			</div>			
			<div class="btn-group pull-right hidden-phone">
		...
		?>
	        <tr>
	            ...
	            <td valign="center" >
	                <?php
					if($item->img_name){
						$path= JURI::root(true).$item->img_path.$item->img_name.'_ths.'.$item->img_ext;					
					}else{
				...
				<td>
					...
					<?php $cat_full_path = strip_tags(DJClassifiedsCategory::getCatFullPath($item->cat_id)); ?>
					<?php if($cat_full_path != $item->cat_name){ ?>
						<br><span class="cat_path" style="color:#666;font-size:11px"><?php echo str_replace(' - '.$item->cat_name, '', $cat_full_path); ?></span>
					<?php } ?>
	            </td>
	            ...
	            <td>
	            	<?php 
						if($item->user_id){
							echo '<a href="index.php?option=com_djclassifieds&task=profile.edit&id='.$item->user_id.'">'.$item->user_name.' ('.$item->user_id.')'.'</a>';
						}else{
							echo JText::_('COM_DJCLASSIFIEDS_OPTION_GUEST');
						}
	            	?>
	            </td>
	            <?php if($par->get('abuse_reporting','0')==1){?>
	            ...
	            <td align="center" class="published">
					<?php echo JHtml::_('jgrid.published', $item->published, $i, 'items.', true, 'cb'	); ?>
					<?php if($item->blocked){ ?>
						<span class="icon icon-minus" title="<?php echo JText::_('COM_DJCLASSIFIEDS_BLOCKED'); ?>"> </span>
					<?php } ?>
	            </td>
	             <td align="center" class="date">
					<?php 
					if($item->date_exp == '2038-01-01 00:00:00'){
						echo '<span style="color:#559D01;font-weight:bold;" >'.JText::_('COM_DJCLASSIFIEDS_UNLIMITED').'</span>';
