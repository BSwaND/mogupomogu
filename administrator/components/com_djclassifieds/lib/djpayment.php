...
class DJClassifiedsPayment {
	...
	public static function confirmUserPayment($item, $type,$price_total,$method,$coupon_code=''){
		...
		if($type=='plan'){
			...
			$query = "INSERT INTO #__djcf_plans_subscr (`user_id`,`plan_id`,`adverts_limit`,`adverts_available`,`date_start`,`date_exp`,`plan_params`) "
					."VALUES ('".$user->id."','".$plan->id."','".$plan_params->ad_limit."','".($plan_params->ad_limit == '-1' ? '9999999' : $plan_params->ad_limit)."','".$date_start."','".$date_exp."','".addslashes($plan->params)."')";
			$db->setQuery($query);
			$db->query();
			...
			$app->redirect($redirect, $message);
				...
				$query = "INSERT INTO #__djcf_users_points (`user_id`,`points`,`description`) "
						."VALUES ('".$user->id."','".$item->points."','".addslashes(JText::_('COM_DJCLASSIFIEDS_POINTS_PACKAGE')." <br />".JText::_('COM_DJCLASSIFIEDS_PAYMENT_ID')).' '.$row->id."')";
				$db->setQuery($query);
				$db->query();
	...
	public static function processPayment($id, $type,$method){
		...
		$dispatcher->trigger('onInitProcessPayment', array(&$par, $id, $type, $method));
		...
		if($type=='prom_top'){
			...
			$query = "SELECT d.* FROM #__djcf_days d "
				...
				."WHERE d.published=1 AND d.days=".$item->exp_days." AND (dx.cat_id=".$item->cat_id." OR dx.cat_id IS NULL) "
		...
		$user	= JFactory::getUser();
			...
			$payment = $db->loadObject();
				...
				if($payment){
					...
					$query = "SELECT count(*) FROM #__djcf_payments WHERE id=".$id." AND status='Completed'";
					$db->setQuery($query);
					...
					$query = "UPDATE #__djcf_payments SET status='Completed',transaction_id='".$txn_id."' "
							...
							if($payment->type==5){	//offer
								...
								$query = "INSERT INTO #__djcf_plans_subscr (`user_id`,`plan_id`,`adverts_limit`,`adverts_available`,`date_start`,`date_exp`,`plan_params`) "
										."VALUES ('".$payment->user_id."','".$plan->id."','".$plan_params->ad_limit."','".($plan_params->ad_limit == '-1' ? '9999999' : $plan_params->ad_limit)."','".$date_start."','".$date_exp."','".addslashes($plan->params)."')";
										$db->setQuery($query);
										$db->query();
