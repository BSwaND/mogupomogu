...
class DJClassifiedsControllerAddItem extends JControllerLegacy {
		...
		header("Content-type: text/html; charset=utf-8");
		 ...
		 if(count($fields_list)==0){
		 	...
		 	foreach($fields_list as $fl){		 		
				...
				echo '<div class="djform_row djrow_'.$fl->name.' '.$fl->class.'">';
				if($fl->type=="inputbox"){
						if($id>0){
							$fl_value = $fl->value; 	
						}else{
						...
						if($fl_value_to == '0000-00-00') $fl_value_to = '';
						echo JHTML::calendar($fl_value_to,$fl->name.'_to', 'dj'.$fl->name.'_to', DJClassifiedsTheme::dateFormatConvert($fl->date_format),array('size'=>'10','maxlength'=>'19','class'=>$cl));
	...
	public function getRegionSelect(){
		...
		if($id>0){
			...
			if($regions){													
					echo '<div class="clear_both"></div>';
						echo '<select name="regions[]" id="reg_'.$id.'" onchange="new_reg('.$id.',this.value,new Array());">';
	...
	public function getCategorySelect(){
		...
		if($id>0){
			...
			$dispatcher->trigger('onDJClassifiedsItemEditGetCategorySelect', array(&$cats, &$par, &$subscr_id, &$mc));
			...
			if($cats){
				...
				echo '<div class="clear_both"></div>';
				if($mc!=''){
					echo '<select '.$cl_select.'  name="mcats'.$mc.'[]" id="mcat'.$mc.'_'.$id.'" onchange="new_mcat('.$id.',this.value,new Array(),'.$mc.');getFields(this.value, true);">';
				}else{
					echo '<select '.$cl_select.' name="cats[]" id="cat_'.$id.'" onchange="new_cat('.$id.',this.value,new Array());getFields(this.value,false);">';
				}
	...
	function save(){
		...
		$model->checkCatAllowed();
		if(!$id){
			$model->checkAdsLimits();
			$model->checkCatAdsLimits();
		}
	...
	*/
	...
	$duration_price =0;
		if($row->id==0){			
			if($par->get('durations_list','')){
				...
				."LEFT JOIN #__djcf_days_xref dx ON d.id=dx.day_id "
				."WHERE d.published=1 AND d.days=".$exp_days." AND (dx.cat_id=".$row->cat_id." OR dx.cat_id IS NULL) "
				."ORDER BY dx.cat_id DESC LIMIT 1";
				$db->setQuery($query);								
		...
		if(($row->region_id || $row->address) && (($row->latitude=='0.000000000000000' && $row->longitude=='0.000000000000000') || (!$row->latitude && !$row->longitude))){			
			...
			*/
			...
			if($row->post_code){
				$loc_coord = DJClassifiedsGeocode::getLocationPostCode($row->post_code, $address);
			}else{
				$loc_coord = DJClassifiedsGeocode::getLocation($address);
			}
		 ...
		 if(count($fields_list)>0){
				...
				foreach($fields_list as $fl){
					if($fl->type=='checkbox'){
						...
						if(isset($_POST[$fl->name])){							
							...
							if($f_var && $f_var!='0000-00-00 00:00:00' && $fl->date_format){
								$f_var = date_format(date_create_from_format($fl->date_format, $f_var), 'Y-m-d');
							}
						...
						if(isset($_POST[$fl->name]) || isset($_POST[$fl->name.'_to'])){							
							...
							if($f_var_to && $f_var_to!='0000-00-00 00:00:00' && $fl->date_format){
								$f_var_to = date_format(date_create_from_format($fl->date_format, $f_var_to), 'Y-m-d');
							}
						...
						if(isset($_POST[$fl->name.'_start']) || isset($_POST[$fl->name.'_end'])){
							...
							if($f_var_start && $f_var_start!='0000-00-00 00:00:00' && $fl->date_format){
								$f_var_start = date_format(date_create_from_format($fl->date_format, $f_var_start), 'Y-m-d');
							}
							$f_var_end = JRequest::getVar( $fl->name.'_end','','','string' );
							if($f_var_end && $f_var_end!='0000-00-00 00:00:00' && $fl->date_format){
								$f_var_end = date_format(date_create_from_format($fl->date_format, $f_var_end), 'Y-m-d');
							}
		  ...
		  //print_r($query);die();
			...
			if($par->get('promotion','1')=='1'){
				...
				foreach($promotions as $prom){
					...
					if($ins_prom){
						...
						."('".$row->id."','".$prom->id."','".$prom_exp_date."','".$days_left_default."')";
			...
			if(!($user->id && $par->get('ad_preview','0') && JRequest::getInt('preview_value',0))){
				...
				if($new_ad && $par->get('user_new_ad_email','0') && ($user->id>0 || ($par->get('email_for_guest','0') && $row->email))){						
	...
	public function publish(){				
		...
		if($item->new_draft){
			...
			if($par->get('user_new_ad_email','0') && ($user->id>0 || ($par->get('email_for_guest','0') && $item->email))){						
				DJClassifiedsNotify::notifyNewAdvertUser($item,'');
			}					
			$query ="UPDATE #__djcf_items SET new_draft=0 WHERE id=".$item->id;
			$db->setQuery($query);
			$db->execute();
		...
		if($item->user_id!=$user->id && $user->authorise('core.admin', 'com_djclassifieds')){
			$query ="UPDATE #__djcf_items SET published=1 WHERE id=".$item->id." ";
			$db->setQuery($query);
			$db->query();					
			$message = JText::_('COM_DJCLASSIFIEDS_AD_SAVED_SUCCESSFULLY');	
		}elseif($item->pay_type && $item->payed==0 ){
			$menu_uads_itemid = $menus->getItems('link','index.php?option=com_djclassifieds&view=useritems',1);
	...
	public function getDurationSelect(){
		...
		$query = "SELECT d.*, IFNULL(c.cat_c,0) AS cat_c, group_ids FROM #__djcf_days d "
				."LEFT JOIN (SELECT COUNT(id) as cat_c, day_id FROM #__djcf_days_xref GROUP BY day_id) c ON c.day_id=d.id "
				."LEFT JOIN (SELECT d.id, GROUP_CONCAT(dg.group_id) group_ids FROM #__djcf_days d LEFT JOIN #__djcf_days_groups dg ON d.id=dg.day_id GROUP BY d.id) g ON g.id=d.id "
				...
				foreach($days as $key => $day){
					if($day->group_ids){ // restricted access
						$d_ug_arr = explode(',', $day->group_ids);
						$matched = false;
						foreach($d_ug_arr as $d_ug){
							if(in_array($d_ug, $ug_arr)){
								$matched = true;
							}
						}
						if(!$matched){
