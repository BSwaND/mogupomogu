...
<script type="text/javascript">	
	...
	function getCities(region_id){
		...
		if(region_id>0){
			...
			var url = '<?php echo JURI::base()?>index.php?option=com_djclassifieds&view=additemtask=getCities&r_id=' + region_id <?php if(!empty($this->profile->id)){echo "+ '&id='+".$this->profile->id;} ?>;
				var myRequest = new Request({
					    url: '<?php echo JURI::base()?>index.php',
					    method: 'post',
						data: {
					      ...
					      'task': 'getCities',
						  'r_id': region_id
						  <?php if(!empty($this->profile->id)){echo ",'id':'".$this->profile->id."'";} ?>					  
						  },
					    onSuccess: function(responseText){																
