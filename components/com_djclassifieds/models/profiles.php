...
class DjclassifiedsModelProfiles extends JModelLegacy{	
	...
	function getItems(){
		...
		if(JRequest::getVar('p_search','','','string')){							
			...
			$group_id = JRequest::getInt('p_se_group_id','');
			if($group_id){
				$search .=" AND p.group_id=".$group_id." ";
			}
		...
		$query = "SELECT u.id as u_id, u.name, u.username, u.email, p.*, img.path, r.name as r_name ".$search_radius_v." "
				...
				."LEFT JOIN (SELECT item_id, CONCAT(path,name,'_ths','.',ext) path FROM #__djcf_images WHERE type='profile') img ON p.user_id=img.item_id "
				...
				."WHERE p.user_id=u.id AND u.block=0 AND (u.activation = '0' OR u.activation = '') ".$search.$search_radius_h
				."ORDER BY ".$ord;
	//print_r($query);//die();
