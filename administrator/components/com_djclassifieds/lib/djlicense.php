	...
	public static function getSubscription($name){
		...
		if(strstr($res[0], 'E')){
			...
			if ($version < $res[1]) {
				...
				$version_message = 'COM_DJCLASSIFIEDS_DJLIC_VERSION_OLD';
				$component_version = 'COM_DJCLASSIFIEDS_DJLIC_YOUR_VERSION';
				$latest_version = '<span class="djlic_latest_version">'.JText::_('COM_DJCLASSIFIEDS_DJLIC_NEW_VERSION').' <span>'.$res[1].'</span></span>';
			} else {
				$icon_class = 'djlic_icon_valid';
				$version_message = 'COM_DJCLASSIFIEDS_DJLIC_VERSION_LATEST';
				$component_version = 'COM_DJCLASSIFIEDS_DJLIC_VERSION';
				$latest_version = '';
			}
			
			$update = '<div class="djlic_box">';
				$update .= '<div class="djlic_logo_wrapper">';
				$update .= '<div class="djlic_logo"><img alt="DJ-'.$name.'" src="'.JURI::base().'components/com_djclassifieds/assets/images/dj-classifieds.svg" /></div>';
				...
				$update .= '<a href="https://dj-extensions.com/changelog/dj-classifieds/" target="_blank">'.JText::_('COM_DJCLASSIFIEDS_DJLIC_CHANGELOG').'</a>';
				$update .= '</div>';
				$update .= '</div>';
				
				if($res[0]=='E4'){

					$update .= '<div class="djlic_expired">';
					$update .= '<span class="djlic_icon djlic_icon_warning"></span>';
					$update .= '<div class="djlic_info">';
					$update .= '<p>'.JText::_('COM_DJCLASSIFIEDS_DJLIC_EXPIRED_INFO').'</p>';
					$update .= '<input id="license" type="text" name="license" class="input input-large" value="'.$license.'" disabled /><br />';
					$update .= '<a class="btn btn-djex" target="_blank" href="https://dj-extensions.com/my-licenses">'.JText::_('COM_DJCLASSIFIEDS_DJLIC_RENEW').'</a>';
					...
					$update .= '<span class="djlic_icon djlic_icon_invalid"></span>';
					$update .= '<div class="djlic_info">';
					$update .= '<p>'.JText::_('COM_DJCLASSIFIEDS_DJLIC_ENTER_LICENSE_CODE').' <a href="//'.$u->getHost().'" target="_blank">'.$u->getHost().'</a></p>';
					$update .= '<input id="license" type="text" name="license" class="input input-large" placeholder="'. JText::_('DJUPDATER_PASTE_KEY').'" /><br />';
				...
				jQuery(document).ready(function(){
					...
					button.click(function(e){
						...
						jQuery.ajax({
							...
							button.closest('.djlic_invalid, .djlic_expired').prepend(jQuery(data));
			...
			if ($version < $res[3]) {
				...
				$version_message = 'COM_DJCLASSIFIEDS_DJLIC_VERSION_OLD';
				$component_version = 'COM_DJCLASSIFIEDS_DJLIC_YOUR_VERSION';
				$latest_version = '<span class="djlic_latest_version">'.JText::_('COM_DJCLASSIFIEDS_DJLIC_NEW_VERSION').' <span>'.$res[3].'</span></span>';
			} else {
				$icon_class = 'djlic_icon_valid';
				$version_message = 'COM_DJCLASSIFIEDS_DJLIC_VERSION_LATEST';
				$component_version = 'COM_DJCLASSIFIEDS_DJLIC_VERSION';
			...
			$update_available = version_compare($version, $res[3], '<');
			...
			$update .= '<span class="djlic_icon djlic_icon_valid"></span>';
			$update .= '<div class="djlic_info">';
			$update .= '<p>'.JText::sprintf('COM_DJCLASSIFIEDS_DJLIC_VALID_LICENSE_INFO', date("Y-m-d", strtotime($res[2]))).'</p>';
			$update .= '<input id="license" type="text" name="license" class="input input-large" value="'.$license.'" disabled /><br />';
			$update .= '<a class="btn btn-djex" target="_blank" href="https://dj-extensions.com/my-licenses">'.JText::_('COM_DJCLASSIFIEDS_DJLIC_EXTEND').'</a>';
			$update .='</div>';
			$update .='</div>';
