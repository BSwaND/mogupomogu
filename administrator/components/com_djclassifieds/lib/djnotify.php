...
class DJClassifiedsNotify {
				...
				foreach($items as $i){
					...
					if (filter_var($mailto, FILTER_VALIDATE_EMAIL)) {
						...
						if (!is_object($send)){
							...
							$app->enqueueMessage('Sending expiration notification emails failed', 'warning');
							break;
						}						
					}else{ 
						$update_id .= $i->id.', ';
	...
	public static function messageAskFormContact($item,$author,$message,$files,$replyto,$replytoname,$custom_fields_msg){
		...
		$app->triggerEvent('onDJClassifiedsPrepareEmail', array($email, $item, $reciver, $message, $author, $custom_fields_msg));
	...
	public static function notifyUserPaymentCompleted($payment){
		...
		$subject = self::parseMessageBody($email->title,$email->id,$item,$receiver,'','','','','','','','','','','',$payment_info,'',$offer);
		...
		$mailer->IsHTML(true);
		$mailer->addRecipient($mailto);
		$dispatcher->trigger('onNotifyUserPaymentCompletedPrepareMailer', array(&$mailer, $payment->id));
		//echo '<pre>';print_r(array($mailfrom, $fromname, $subject, $message, $mailto));die();
		$mailer->Send();
	...
	public static function notifyUserPoints($user_id,$points_info){
		...
		$subject = self::parseMessageBody($email->title,$email->id,'',$reciver,'','','','','','','','','','','','',$points_info);
		$message = self::parseMessageBody($email->content,$email->id,'',$reciver,'','','','','','','','','','','','',$points_info);
	...
	public static function notifyAdminPaymentPoints($type,$payment_info){
		...
		$subject = self::parseMessageBody($email->title,$email->id,'',$reciver,'','','','','','','','','','','',$payment_info);
		$message = self::parseMessageBody($email->content,$email->id,'',$reciver,'','','','','','','','','','','',$payment_info);
	...
	public static function messageProfileAskFormContact($profile,$author,$message,$files,$replyto,$replytoname,$custom_fields_msg){
		...
		$app->triggerEvent('onDJClassifiedsPrepareEmail', array($email, null, $reciver, $message, $author, $custom_fields_msg));
		...
		if($par->get('ask_seller_copy_sender','0')==1){
			...
			$subject_sender = self::parseMessageBody($email_sender->title,$email_sender->id,'',$reciver,'','','','',$message,$author,'','','','',$custom_fields_msg);
			$message_sender = self::parseMessageBody($email_sender->content,$email_sender->id,'',$reciver,'','','','',$message,$author,'','','','',$custom_fields_msg);
			$dispatcher->trigger('onDJClassifiedsSendCopyMessage', array('',$author,$author['email'],$mailfrom,$fromname,'','',$subject_sender,$message_sender,$files,$custom_fields_msg));
		}
	...
	public static function getPurchaseDetails($user_id){
		...
		if($pd_field_id_c){
			$query = "SELECT p.value FROM #__djcf_fields_values_profile p "
					."WHERE p.field_id = '".$pd_field_id_c."' AND p.user_id = ".$user_id." ";
			$db->setQuery($query);
	...
	public static function parseMessageBody($message,$message_id,$item,$reciver='',$bidder='',$bid='',$bcontact_message='',$bowner='',$contact_message='',$contact_author='',$abuse_message='',$abuse_author='',$buyer='',$quantity='',$contact_fields_message='',$payment_info=array(),$points_info=array(),$offer=array(),$sa=array()){		
		...
		if($item){
			...
			$message = str_ireplace('[[advert_category_path]]', DJClassifiedsCategory::getCatFullPath($item->cat_id), $message);
		...
		if($payment_info){
			...
			$message = str_ireplace('[[payment_id]]', $payment_info['payment_id'], $message);
