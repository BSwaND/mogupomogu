	...
	if(count($this->fields)>0){?>
		<div class="custom_det<?php echo $cd_class;?>">
			...
			<div class="custom_det_content">
				...
				<?php if($item->event->afterDJClassifiedsDisplayExtraFields) { ?>
					<div class="djcf_after_extra_fields">
						<?php echo $this->item->event->afterDJClassifiedsDisplayExtraFields; ?>
					</div>
				<?php } ?>
			</div>	
		</div>

		<?php if($app->input->get('tmpl', '') == 'component'){ // showing phone numbers on print view ?>
			<script>
				jQuery(function($){
