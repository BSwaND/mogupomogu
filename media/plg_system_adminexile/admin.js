var AdminExile = new Class({
    ...
    initialize:function(options){
        ...
        ['label[for=jform_params_twofactor0]','label[for=jform_params_twofactor1]'].each(function(item){
...
//        if(document.id('jform_params_keyvalue').isVisible()) {
            adminurl.setData({});           
...
//        target = document.id('jform_params_url-lbl').getParent(this.options.parentelement).getElements('span.after')[0];
        target = document.id('jform_params_url-lbl').getParent('div.control-group').getElements('span.after')[0];
        target.empty();
