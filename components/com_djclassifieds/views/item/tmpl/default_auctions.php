...
<?php if($par->get('auction_timer','0')){ ?>
	...
	<script>
		...
		window.addEvent('domready', function(){
			...
			if(document.getElementById('djtimer') && !document.getElementsByClassName('bids_close_info').length){
				...
				var x = setInterval(function() {
				  if(!document.getElementById('djtimer')) return;
				  ...
				  if (distance < 0) {
				    clearInterval(x);
				    document.getElementById('djtimer').innerHTML = '<span><?php echo DJClassifiedsTheme::sanitizeText(JText::_('COM_DJCLASSIFIEDS_AUCTION_TIMER_EXPIRED')); ?></span>';
				  }
				}, 1000);
			}
		});
