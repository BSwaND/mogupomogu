				...
				<fieldset class="adminform">
					...
					<div class="tab-content">
						<div class="tab-pane active" id="details">
							...
							<?php $cl = ($this->field->source>1 && $this->field->source!=3 ? ' style="display:none;" ' : '');?>	
							...
							<div id="core_source" class="control-group" <?php echo $cl;?>>
								...
								<div class="controls">
									<select name="core_source" class="inputbox">
										...
										<option value="post_code" <?php if($this->field->core_source=='post_code'){ echo 'SELECTED'; }?> ><?php echo JText::_('COM_DJCLASSIFIEDS_POSTCODE');?></option>
										<option value="video" <?php if($this->field->core_source=='video'){ echo 'SELECTED'; }?> ><?php echo JText::_('COM_DJCLASSIFIEDS_VIDEO');?></option>
										<option value="website" <?php if($this->field->core_source=='website'){ echo 'SELECTED'; }?> ><?php echo JText::_('COM_DJCLASSIFIEDS_WEBSITE');?></option>
									</select>					
								</div> 
							</div>
							<div class="control-group">
								<div class="control-label">
		                    		...
		                    		<br /><span style="color:#666;width:160px;display:inline-block;"><b>Deprecated</b><br />For custom styling purposes, use "CSS class" parameter</span>
								</div>
								<div class="controls">
							...
							<div class="control-group">
								<div class="control-label">
									<?php echo JText::_('COM_DJCLASSIFIEDS_FIELD_CLASS');?>
								</div>
								<div class="controls">
		                   			<input class="text_area" type="text" name="class" id="class" value="<?php echo $this->field->class; ?>" />									
								</div>
							</div>
						...
						<div class="tab-pane" id="search">
							...
							<div class="control-group" id="search_on_start_box"  style="<?php echo $cl_in_search_box; ?>">
								...
								<div class="controls">
									<input type="radio" autocomplete="off" name="in_search_on_start" value="1" <?php  if($this->field->in_search_on_start==1){echo "checked";}?> /><span style="margin:2px 10px 0 5px;float:left;"><?php echo JText::_('COM_DJCLASSIFIEDS_YES'); ?></span>
									<input type="radio" autocomplete="off" name="in_search_on_start" value="0" <?php  if($this->field->in_search_on_start==0 || !$this->field->id){echo "checked";}?> /><span style="margin:2px 10px 0 5px;float:left;"><?php echo JText::_('COM_DJCLASSIFIEDS_NO'); ?></span>
								</div>
							</div>
							...
							<?php if($search_type=='checkbox' || $search_type=='checkbox_accordion_o' || $search_type=='checkbox_accordion_c'){
								$st= "";
							}else{ $st='display:none;'; } ?>
							<div class="control-group" id="chx_filter_logic_box" style="<?php echo $st;?>" >
								<div class="control-label">
									<?php echo JText::_('COM_DJCLASSIFIEDS_CHECKBOX_FILTER_LOGIC'); ?>
									<br /><span style="color:#666;width:160px;display:inline-block;"><?php echo JText::_('COM_DJCLASSIFIEDS_CHECKBOX_FILTER_LOGIC_DESC');?></span>
								</div>
								<div class="controls">
									<select name="chx_filter_logic" id="chx_filter_logic">
									<option value="0" <?php if($this->field->chx_filter_logic=='0'){ echo 'selected';}?> ><?php echo JText::_('COM_DJCLASSIFIEDS_CHECKBOX_FILTER_LOGIC_OR'); ?></option>
										<option value="1" <?php if($this->field->chx_filter_logic=='1'){ echo 'selected';}?> ><?php echo JText::_('COM_DJCLASSIFIEDS_CHECKBOX_FILTER_LOGIC_AND'); ?></option>
									</select>
								</div>
							</div>							
						</div>
...
<script type="text/javascript">
	function inSearch(v){
		if(v=='1'){
			...
			if(search_type=='select' || search_type=='select_min_max' || search_type=='checkbox' || search_type=='checkbox_accordion_o' || search_type=='checkbox_accordion_c'){
				$('search_value1_box').setStyle("display","");
			}
			if(search_type=='select_min_max'){
				$('search_value2_box').setStyle("display","");
			}
			if(search_type=='checkbox' || search_type=='checkbox_accordion_o' || search_type=='checkbox_accordion_c'){
				$('chx_filter_logic_box').setStyle("display","");
			}
		}else{
			...
			$('chx_filter_logic_box').setStyle("display","none");
		}
	}
	...
	function searchType(v){
		...
		if(v=='checkbox' || v=='checkbox_accordion_o' || v=='checkbox_accordion_c'){
			$('chx_filter_logic_box').setStyle("display","");
		}else{
			$('chx_filter_logic_box').setStyle("display","none");
		}
	...
	function confirm_add(title,id){
		...
		var answer = confirm ('<?php echo DJClassifiedsTheme::sanitizePopupText(JText::_('COM_DJCLASSIFIEDS_CONFIRM_ADD_FIELDS_TO_CATEGORIES')); ?>'+' "'+title+'"');
		if (answer){
	...
	function confirm_delete(title,id){
		...
		var answer = confirm ('<?php echo DJClassifiedsTheme::sanitizePopupText(JText::_('COM_DJCLASSIFIEDS_CONFIRM_DELETE_FIELDS_FROM_CATEGORIES')); ?>'+' "'+title+'"');
		if (answer){
	...
	<?php if($this->field->id>0 ){ ?>
		function confirm_add_category(){
			...
			if(cat_id>0){
				var answer = confirm ('<?php echo DJClassifiedsTheme::sanitizePopupText(JText::_('COM_DJCLASSIFIEDS_CONFIRM_ADD_FIELDS_TO_SELECTED_CATEGORIES')); ?>');
				if (answer){
