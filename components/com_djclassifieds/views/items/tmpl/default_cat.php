...
<div id="dj-classifieds" class="clearfix djcftheme-<?php echo $this->theme;?><?php echo $pageclass_sfx;?>">
	<?php 
			...
			if($main_id){
				...
				if(is_array($modules_djcf) && count($modules_djcf)>0){
					echo '<div class="djcf-ad-top-cat clearfix">';
					foreach (array_keys($modules_djcf) as $m){
						echo JModuleHelper::renderModule($modules_djcf[$m],$mod_attribs);
					}
	...
	if($se>0){
		...
		if(!$active_m || ($active_m && $active_m->params->get('show_page_heading','0'))){
			echo '<h1 class="main_cat_title">';
				if($active_m && $active_m->params->get('page_heading','')){
					echo $active_m->params->get('page_heading','');	
				}elseif($active_m && $active_m->params->get('page_title','')){
					echo $active_m->params->get('page_title','');
				}else{
					echo $this->main_reg->name;
				}
			echo '</h1>';
		}
	}else{
		if($Itemid>0){
			$active_m = $app->getMenu('site')->getActive();
			if($active_m && $active_m->params->get('show_page_heading','0')){
				echo '<h1 class="main_cat_title">';
					if($active_m->params->get('page_heading','')){
						echo $active_m->params->get('page_heading','');	
					}elseif($active_m->params->get('page_title','')){
						echo $active_m->params->get('page_title','');
					}else{
						echo $active_m->title;
					}
		...
		if($par->get('main_cat_desc','0') && !empty($this->main_cat->description)){
