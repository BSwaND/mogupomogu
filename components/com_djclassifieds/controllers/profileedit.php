...
class DJClassifiedsControllerProfileedit extends JControllerLegacy {
	...
	function save(){
		...
		$requestData['name'] = $app->input->get('u_name','','STRING');
		$requestData['username'] = $user->username;
		$requestData['password1'] = $app->input->get('u_password1','','RAW');
		$requestData['password2'] = $app->input->get('u_password2','','RAW');
		$requestData['email1'] = $app->input->get('u_email1','','STRING');
		$requestData['email2'] = $app->input->get('u_email2','','STRING');
		...
		if ($data === false)
		{
			...
			$this->setRedirect(JRoute::_($user_edit_profile, false));
		...
		if ($return === false)
		{
			...
			// Redirect back to the edit screen.
			$this->setMessage(JText::sprintf('COM_USERS_PROFILE_SAVE_FAILED', $users_model->getError()), 'warning');
			$this->setRedirect(JRoute::_($user_edit_profile, false));
