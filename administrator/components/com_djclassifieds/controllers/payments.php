...
class DJClassifiedsControllerPayments extends JControllerAdmin
{
	...
	*/
	...
	function changeStatus(){
			...
			if($row->type==5){	//offer															
				...
				if($status=='Completed' && $row->status!='Completed'){
					...
					$query = "INSERT INTO #__djcf_plans_subscr (`user_id`,`plan_id`,`adverts_limit`,`adverts_available`,`date_start`,`date_exp`,`plan_params`) "
							."VALUES ('".$row->user_id."','".$plan->id."','".$plan_params->ad_limit."','".($plan_params->ad_limit == '-1' ? '9999999' : $plan_params->ad_limit)."','".$date_start."','".$date_exp."','".addslashes($plan->params)."')";					
					$db->setQuery($query);
					$db->query();						
		...
		if($row->status == 'Completed' && $par->get('notify_payment_complete', '0')){
	...
	function delete(){
		...
		$app->redirect('index.php?option=com_djclassifieds&view=payments', JText::_('COM_DJCLASSIFIEDS_N_ITEMS_DELETED'));
	}
