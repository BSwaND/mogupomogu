...
class DJclassifiedsViewItem extends JViewLegacy{
	...
	function display($tpl = null){		
		...
		if($item->user_id!=0){
			...
			$document->addCustomTag('<meta property="twitter:description" content="'.$item->intro_desc.'" />');
			...
			if($par->get('show_print_icon','0')){
				$print_styles = "#dj-classifieds .dj-item .print_icon{float: right;margin: 0 5px;}.contentpane.modal #dj-classifieds .print_icon,.contentpane.modal #dj-classifieds .button{display:none;}";
				$print_styles .= ".contentpane.modal #dj-classifieds a[href]:after {content: none !important;}";
				$document->addStyleDeclaration($print_styles);
			}
			...
			$resultsAfterExtraFields = $dispatcher->trigger('onAfterDJClassifiedsDisplayExtraFields', array (&$item, & $par, 'item'));
			$item->event->afterDJClassifiedsDisplayExtraFields = trim(implode("\n", $resultsAfterExtraFields));

			$resultsAccessContact = $dispatcher->trigger('onDJClassifiedsAccessContact', array(&$item, &$par, 'item'));
			$item->event->onDJClassifiedsAccessContact = trim(implode("\n", $resultsAccessContact));
