	...
	/*	$menus	= JSite::getMenu();	
		...
		*/		
		...
		if($id){
			...
			$db->setQuery($query);
			$item = $db->loadObject();
		}elseif($user->id){ // if ad not chosen, use first user's ad
			...
			if($item){
				...
				$redirect = JRoute::_(DJClassifiedsSEO::getNewAdLink(), false);
				$app->redirect($redirect, JText::_('COM_DJCLASSIFIEDS_ITEM_NOT_FOUND'), 'warning');
			}
		}
		...
		if(!$advert_available){			
			if($par->get('404_item_redirect','0')==1){
				...
				$app->redirect($redirect,$message, 'warning');
			}
		}
		...
		if($date_now<$item->date_exp || $item->user_id==$user->id || $item->published==2){		
			...
			parent::display();
		}elseif($date_now >= $item->date_exp && $par->get('access_expired','0') && JFactory::getDate('now - '.(int)$par->get('access_expired').' day') < $item->date_exp){
...
function ask(){
	...
	if($send_email){
		$query = 'SELECT i.*,c.name as c_name, c.alias as c_alias, u.name as u_name, u.email as u_email FROM #__djcf_items i '
					.'LEFT JOIN #__djcf_categories c ON c.id=i.cat_id '
...
function delete(){
	...
	$dispatcher = JDispatcher::getInstance();
		...
		if($item->date_exp > date('Y-m-d H:i:s')){
			$query = "SELECT ads_limit FROM #__djcf_categories WHERE id=".$item->cat_id;
			$db->setQuery($query);
			$cat_ads_limit = $db->loadResult();
			if($cat_ads_limit){
				$message = JText::_('COM_DJCLASSIFIEDS_CATEGORY_ADS_LIMIT_DELETE_NOT_ALLOWED');
				$redirect = JRoute::_(DJClassifiedsSEO::getUserAdsLink(), false);
				$app->redirect($redirect, $message,'error');
			}
		}
	...
	function getSearchFields(){
		 ...
		 $resultsFields = $dispatcher->trigger('onBeforeDJClassifiedsDisplaySearchFields', array(&$par));
		 if($resultsFields){
			echo trim(implode("\n", $resultsFields));
		 }
		 ...
		 if(count($fields_list)==0){
		 	...
		 	foreach($fields_list as $fl){
					if($parent_cid != $session->get('last_parent_cid') || !JRequest::getInt('se', 1)){
						$session->set('se_'.$fl->id,'');
						$session->set('se_'.$fl->id.'_min','');
						$session->set('se_'.$fl->id.'_max','');
					}
		 			...
		 			echo '<div class="'.$fl_class.' '.$fl->class.'">';
					...
					if($fl->type=='date' || $fl->type=='date_from_to'){
						if($fl->search_type=='inputbox'){	
							...
							//echo ' <img class="calendar" src="'.JURI::base(true).'/components/com_djclassifieds/assets/images/calendar.png" alt="calendar" id="se_'.$fl->id.'button" />';
							$cl = "inputbox djsecal";
							echo JHTML::calendar($value,'se_'.$fl->id, 'se_'.$fl->id, DJClassifiedsTheme::dateFormatConvert($fl->date_format),array('size'=>'10','maxlength'=>'19','class'=>$cl));
							...
							//echo ' <img class="calendar" src="'.JURI::base(true).'/components/com_djclassifieds/assets/images/calendar.png" alt="calendar" id="se_'.$fl->id.'_minbutton" />';
							$cl = "inputbox djsecal";
							echo JHTML::calendar($value,'se_'.$fl->id.'_min', 'se_'.$fl->id.'_min', DJClassifiedsTheme::dateFormatConvert($fl->date_format),array('size'=>'10','maxlength'=>'19','class'=>$cl));
							echo '<br />';
							...
							//echo ' <img class="calendar" src="'.JURI::base(true).'/components/com_djclassifieds/assets/images/calendar.png" alt="calendar" id="se_'.$fl->id.'_maxbutton" />';
							echo JHTML::calendar($value,'se_'.$fl->id.'_max', 'se_'.$fl->id.'_max', DJClassifiedsTheme::dateFormatConvert($fl->date_format),array('size'=>'10','maxlength'=>'19','class'=>$cl));
						}
					}else{
						if($fl->search_type=='inputbox'){	
							if($session->get('se_'.$fl->id,'')!=''){
								...
								for($i=0;$i<count($val);$i++){
									...
									echo '<div class="radiofield_box_v"><input type="radio" class="inputbox" '.$checked.' value ="'.$val[$i].'" name="se_'.$fl->id.'" id="se_'.$fl->id.'_'.$i.'"  />';
										echo '<label for="se_'.$fl->id.'_'.$i.'" class="radio_label">';
											if($val[$i]==''){
												echo JText::_('COM_DJCLASSIFIEDS_FILTER_ALL');
											}else{
												if($par->get('cf_values_to_labels','0')){
													echo JText::_('COM_DJCLASSIFIEDS_'.str_ireplace(' ', '_', strtoupper($val[$i])));
												}else{
							...
							echo '<div class="se_checkbox">';
								for($i=0;$i<count($val);$i++){
									...
									echo '<div class="se_checkbox_v"><input type="checkbox" '.$checked.' value ="'.$val[$i].'" name="se_'.$fl->id.'[]" id="'.$mod_id.'_se_'.$fl->id.'_'.$i.'" />';
										echo '<label for="'.$mod_id.'_se_'.$fl->id.'_'.$i.'" class="radio_label">';
											if($par->get('cf_values_to_labels','0')){
												echo JText::_('COM_DJCLASSIFIEDS_'.str_ireplace(' ', '_', strtoupper($val[$i])));
											}else{
												echo $val[$i];
	...
	function delBid(){
		...
		public function getCategorySelect(){		
			...
			$subcat_label = JRequest::getInt('subcat_label', '0');
				...
				if($id>0 && !strstr($pid, 'p')){
						...
						if($cats){						
							...
							echo '<select class="inputbox" name="se_cats[]" id="se'.$mod_id.'_cat_'.$id.'" onchange="se'.$mod_id.'_new_cat('.$id.',this.value,new Array());se'.$mod_id.'_getFields(this.value);">';
								if($subcat_label==1 && $parent_cat->parent_id==0){
									echo '<option value="p'.$id.'">'.JTEXT::_('MOD_DJCLASSIFIEDS_SEARCH_CATEGORY_LABEL').'</option>';
								}else{
