...
<form action="<?php echo JRoute::_($itemid_url.'&Itemid='.$itemid.'&se=1');?>" method="get" name="form-search<?php echo $module->id?>" id="form-search<?php echo $module->id?>">
	...
	<?php }
	if($params->get('show_account_type',0)==1 && count($custom_fields_groups)){ ?>
		<div class="account_type djcf_se_row">
			<select class="text_area required"  name="p_se_group_id" id="p_se_group_id" />
				<option value=""><?php echo JText::_('MOD_DJCLASSIFIEDS_ACCOUNT_TYPE_LABEL'); ?></option>
				<?php echo JHtml::_('select.options', $custom_fields_groups, 'id', 'name', JRequest::getInt('p_se_group_id',''), true); ?>
			</select>
		</div>
	<?php }	
