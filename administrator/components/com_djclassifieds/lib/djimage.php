...
class DJClassifiedsImage {
	...
	public static function getAdsImages($item_ids, $type='item'){
		$db			= JFactory::getDBO();
	...
	private static function getImgThumbs($img){
		$par = JComponentHelper::getParams( 'com_djclassifieds' );
		...
		$bigth_use_in = $par->get('bigth_use_in', array(5,6,10));

		foreach($th_types as $value => $th_type){
			if(in_array($value, $bigth_use_in)){
				$img->{'thumb_'.$th_type} = $img->path.$img->name.'_thb.'.$img->ext;
			}elseif(in_array($value, $mediumth_use_in)){
				$img->{'thumb_'.$th_type} = $img->path.$img->name.'_thm.'.$img->ext;
			}else{
				$img->{'thumb_'.$th_type} = $img->path.$img->name.'_ths.'.$img->ext;
			}
		}	
	}
