...
class DjclassifiedsModelItem extends JModelLegacy{	
	...
	function getItem(){
		...
		$db= JFactory::getDBO();
		$query = "SELECT i.*, c.id as c_id, c.name as c_name, c.alias as c_alias, c.access_view as c_access_view, c.access_item_view as c_access_item_view,  c.rev_group_id, r.name as r_name, ".$u_name.", u.email as u_email, iu.name as unit_name ".$fav_s.$type_s." FROM #__djcf_categories c, #__djcf_items i "
				.$fav_lj.$type_lj
	...
	*/
	...
	function getFields($cid){
		...
		$query ="SELECT f.*, v.value,v.value_date,v.value_date_to FROM #__djcf_fields f "
				...
				."ON v.field_id=f.id "
				."WHERE fx.cat_id IN (SELECT cat_id FROM #__djcf_items_categories WHERE item_id=".$id." UNION SELECT ".$cid.") "
				."AND f.published=1 AND f.access=0 AND f.name!='price' AND f.name!='contact' ORDER BY fx.ordering, f.ordering";
	...
	function getCustomAskSellerFields(){
		...
		if($id>0){		
			$query ="SELECT f.* FROM #__djcf_fields f "
					."WHERE f.published=1 AND f.source=3 ORDER BY f.ordering";
			$db->setQuery($query);
