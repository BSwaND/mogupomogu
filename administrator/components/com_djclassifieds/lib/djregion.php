	...
	public static function getRegions(){
		...
		if(self::$_regions){
			...
			$query = "SELECT i.region_id, count(i.id) as items_count "
					."FROM #__djcf_items i WHERE i.published=1 AND i.blocked=0 AND i.date_exp > '".$date_now."' GROUP BY i.region_id";
			$db->setQuery($query); 
...
public static function getRegFullPath($rid, $link = false, $delimiter = ' - '){
    ...
    foreach($regs as $key => $reg){
        ...
        if($link){
            $reg_item .= '<a href="'.DJClassifiedsSEO::getRegionRoute($reg->id.':'.$reg->name).'">'.$reg->name.'</a>';
        }else{
            $reg_item .= $reg->name;
        }
        $reg_item .= '</span>';
