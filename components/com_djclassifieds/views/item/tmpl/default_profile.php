						...
						<?php 							
							echo '<a itemprop="url" class="profile_img" href="index.php?option=com_djclassifieds&view=profile&uid='.$uid_slug.$profile_itemid.'">';
								if($par->get('profile_avatar_source','')){
									...
									if($this->profile['img']){
										echo '<img itemprop="image" alt="'.$item->username.' - logo" src="'.JURI::base(true).$this->profile['img']->path.$this->profile['img']->name.'_ths.'.$this->profile['img']->ext.'" />';
									}else{
										echo '<img itemprop="image" alt="'.$item->username.' - logo" style="width:'.$par->get('prof_smallth_width','50').'px" src="'.JURI::base(true).'/components/com_djclassifieds/assets/images/default_profile_s.png" />';	
									}									
								}
							echo '</a>';
						?>
