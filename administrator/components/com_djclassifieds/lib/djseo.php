...
class DJClassifiedsSEO
{
	...
	public static function getSearchResultsMenuItem($params, $module){
		$app = JFactory::getApplication();
		$menus = $app->getMenu('site');
		
		$se_cats = $app->input->get('se_cats', '');
		$se_regs = $app->input->get('se_regs', '');
		...
		$rid = $se_reg_id ? $se_reg_id : $app->input->getInt('rid', 0);

		if($params->get('results_itemid',0)){
			$menu_custom = $menus->getItem($params->get('results_itemid',0));
		}elseif(!$cid && !$rid && $app->input->get('task', '') == 'parsesearch' && $app->input->get('Itemid', '')){
		    $menu_custom = $menus->getItem($app->input->get('Itemid', ''));
		}
		if(!empty($menu_custom) && strpos($menu_custom->link, 'index.php?option=com_djclassifieds&view=items') !== false){
			return $menu_custom;
		}

		$search_url_table = array();
		$search_url_blog = array();
		if($cid && $rid){
			$search_url_table[] = 'index.php?option=com_djclassifieds&view=items&cid='.$cid.'&rid='.$rid;
			$search_url_blog[] = 'index.php?option=com_djclassifieds&view=items&layout=blog&cid='.$cid.'&rid='.$rid;
		}elseif($cid){
			$search_url_table[] = 'index.php?option=com_djclassifieds&view=items&cid='.$cid;
			$search_url_blog[] = 'index.php?option=com_djclassifieds&view=items&layout=blog&cid='.$cid;
			$search_url_table[] = 'index.php?option=com_djclassifieds&view=items&cid='.$cid.'&rid=0';
			$search_url_blog[] = 'index.php?option=com_djclassifieds&view=items&layout=blog&cid='.$cid.'&rid=0';
		}elseif($rid){
			$search_url_table[] = 'index.php?option=com_djclassifieds&view=items&rid='.$rid;
			$search_url_blog[] = 'index.php?option=com_djclassifieds&view=items&layout=blog&rid='.$rid;
			$search_url_table[] = 'index.php?option=com_djclassifieds&view=items&cid=0&rid='.$rid;
			$search_url_blog[] = 'index.php?option=com_djclassifieds&view=items&layout=blog&cid=0&rid='.$rid;
		}
		$search_url_table[] = 'index.php?option=com_djclassifieds&view=items&cid=0&rid=0';
		$search_url_table[] = 'index.php?option=com_djclassifieds&view=items&cid=0';
		$search_url_table[] = 'index.php?option=com_djclassifieds&view=items';
		$search_url_blog[] = 'index.php?option=com_djclassifieds&view=items&layout=blog&cid=0&rid=0';
		$search_url_blog[] = 'index.php?option=com_djclassifieds&view=items&layout=blog&cid=0';
		$search_url_blog[] = 'index.php?option=com_djclassifieds&view=items&layout=blog';

		if($params->get('result_view','0')==1 || ($app->input->get('task', '') == 'parsesearch' && $app->input->get('layout', '') == 'blog')){
			$search_url = array_merge($search_url_blog, $search_url_table);
		}else{
			$search_url = array_merge($search_url_table, $search_url_blog);
		}

		foreach($search_url as $s_url){
	...
	public static function getSearchResultsLink($params, $module){
		$menu_item = self::getSearchResultsMenuItem($params, $module);
		
		if($menu_item){
			$itemid_url = $menu_item->link;
			$itemid = $menu_item->id;

			$link = JUri::getInstance($itemid_url);
	...
	public static function getSearchResultsItemid($params, $module){
		$menu_item = self::getSearchResultsMenuItem($params, $module);
	...
	public static function getItemsSearchLink($params, $module){ // backward compatibility
		return self::getSearchResultsLink($params, $module);
	}
}
?>
