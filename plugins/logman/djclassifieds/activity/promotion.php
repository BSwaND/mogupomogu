 ...
 */
class PlgLogmanDJClassifiedsActivityPromotion extends ComLogmanModelEntityActivity
{
    protected function _initialize(KObjectConfig $config)
    {
        $config->append(array(
            'format'        => '<em>{actor}</em> has {action} the <em>{object.type}</em> titled <strong>{object}</strong>',
            'object_table'  => 'djcf_promotions',
            'object_column' => 'id')
        );
    ...
    {
        $config->append(array(
            'type' => array('objectName' => 'Promotion', 'object' => true)
        ));
