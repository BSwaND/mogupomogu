	...
	foreach($this->custom_contact_fields as $fl){
		...
		if($fl->type=="inputbox"){
			...
			for($i=0;$i<count($val);$i++){
				...
				echo '<div style="float:left;"><input type="radio" '.$cl.'  '.$checked.' value ="'.$val[$i].'" name="'.$fl->name.'" id="dj'.$fl->name.$i.'" /><label class="radio_label" for="dj'.$fl->name.$i.'">'.$val[$i].'</label></div>';
			...
			for($i=0;$i<count($val);$i++){
				...
				echo '<div style="float:left;"><input type="checkbox" id="dj'.$fl->name.$i.'" class="checkbox" '.$checked.' value ="'.$val[$i].'" name="'.$fl->name.'[]"/><label class="radio_label" for="dj'.$fl->name.$i.'">'.$val[$i].'</label></div>';
				echo '<div class="clear_both"></div>';
			}
			echo '</fieldset>';
			echo '</div>';
