...
class DJClassifiedsViewItems extends JViewLegacy{
	...
	function display($tpl = null){
		...
		if($uid>0 || $se || JRequest::getCmd('order', '') || JRequest::getCmd('ord_t', '')){
		...
		if($m_active && $page_title = $m_active->params->get('page_title','')){
			$document->setTitle($page_title);
		}
	...
	//	$main_name = $model->makePathway($cat_list);
		...
		$dispatcher->trigger('onPrepareDJClassifiedsItems', array(&$par, &$items, &$countitems));
		foreach($items as $item){
			...
			$resultsCategory = $dispatcher->trigger('onDJClassifiedsDisplayCategory', array (&$item, &$par, $event_view));
