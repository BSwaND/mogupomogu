...
class DJClassifiedsControllerField extends JControllerLegacy {
	...
	public function save(){
    	...
    	if (!$row->bind(JRequest::get('post')))
    	{
			...
			if(!$row->name){
				$row->name = JFilterOutput::stringURLSafe($row->label);
			}
			$row->name=strtolower($row->name);
			$row->name=str_ireplace(' ', '_', $row->name); 			
			$row->name=preg_replace("/[^a-z0-9_]/", "", $row->name );

			if(in_array($row->name, array('name', 'website', 'video', 'description', 'intro_desc', 'post_code', 'address', 'latitude', 'longitude', 'type_id', 'exp_days', 'metadesc', 'metakey', 'email', 'currency', 'price_negotiable', 'terms_and_conditions', 'privacy_policy', 'gdpr_agreement', 'preview_value', 'option', 'id', 'token', 'view', 'task'))){
				$app->enqueueMessage('Field name has been changed as it can\'t be the same as the name of the core field', 'warning');
				$row->name = $row->name.'_1';
			}
