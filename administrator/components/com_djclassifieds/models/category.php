...
class DJClassifiedsModelCategory extends JModelAdmin
{
	...
	function getGroups(){
		$id = JRequest::getVar('id', '', '0', 'int');
		$db= JFactory::getDBO();
		$query = "SELECT ug.*, IF((cg.id IS NULL), 0, 1) as active FROM #__usergroups ug " 
				."LEFT JOIN (SELECT * FROM #__djcf_categories_groups WHERE cat_id=".$id." ) cg ON cg.group_id=ug.id "
				."ORDER BY ug.title ";
		$db->setQuery($query);
		$allelems=$db->loadObjectList();
