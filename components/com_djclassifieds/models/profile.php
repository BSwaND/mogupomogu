...
class DjclassifiedsModelProfile extends JModelLegacy{	
	...
	function getProfile($uid){						
			...
			$profile['id'] = $uid;
				...
				$query = "SELECT f.*, v.value, v.value_date, v.value_date_to FROM #__djcf_fields f INNER JOIN "
						."( " 
						."SELECT f.* FROM #__djcf_fields f INNER JOIN #__djcf_profiles p ON f.group_id=p.group_id AND p.user_id=".$uid." AND f.published=1 AND f.source=2 AND f.access=0 "
						."UNION "
						."SELECT f.* FROM #__djcf_fields f WHERE f.published=1 AND f.source=2 AND group_id=0 AND f.access=0 "
						.") f2 ON f.id=f2.id "
						."LEFT JOIN #__djcf_fields_values_profile v ON v.field_id=f.id AND v.user_id=".$uid." "
						."ORDER BY f.ordering";
				$db->setQuery($query);
