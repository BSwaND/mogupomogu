...
class DJClassifiedsControllerRegistration extends JControllerLegacy {	
	...
	function save(){
		...
		$djp_row->bind(JRequest::get('post'));

		$djp_row->user_id = $user_id;

		$reg_post = $app->input->get('regions',array(),'array');
			
		$djp_row->region_id= end($reg_post);
		if(!$djp_row->region_id){
			$djp_row->region_id =$reg_post[count($reg_post)-2];
		}
		if(($djp_row->region_id || $djp_row->address) && (($djp_row->latitude=='0.000000000000000' && $djp_row->longitude=='0.000000000000000') || (!$djp_row->latitude && !$djp_row->longitude))){
			$address= '';
			if($djp_row->region_id){
				$reg_path = DJClassifiedsRegion::getParentPath($djp_row->region_id);
			...
			if($address){	$address .= ', ';}
			$address .= $djp_row->address;
			if($djp_row->post_code){
				$address .= ', '.$djp_row->post_code;
			}
				
			$loc_coord = DJClassifiedsGeocode::getLocation($address);
			if(!empty($loc_coord)){
				$djp_row->latitude = $loc_coord['lat'];
				$djp_row->longitude = $loc_coord['lng'];
			}
		}
		...
		if($group_id>0){
			...
			if (substr($new_avatar['type'], 0, 5) == "image")
			{
				...
				if($img_maxsize && $new_avatar['size'] > $img_maxsize){
					$app->enqueueMessage(JText::_('COM_DJCLASSIFIEDS_TO_BIG_IMAGE').': "'.$new_avatar['name'].'"', 'error');
				}else{
			 ...
			 if(count($fields_list)==0){
			 	...
			 	foreach($fields_list as $fl){
			 		...
			 		if($fl->type=="inputbox"){
							...
							echo JHTML::calendar($fl_value_to,$fl->name.'_to', 'dj'.$fl->name.'_to', '%Y-%m-%d',array('size'=>'10','maxlength'=>'19','class'=>$cl));
