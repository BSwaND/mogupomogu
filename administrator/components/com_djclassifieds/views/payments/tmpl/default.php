...
<form action="index.php?option=com_djclassifieds&view=payments" method="post" name="adminForm" id="adminForm">
	...
	<div id="j-main-container" class="span10">
		<div id="filter-bar" class="btn-toolbar">
			...
			<?php if($this->state->get('filter.type') == '0'){ ?>
			<div class="btn-group pull-right hidden-phone">
				<select name="filter_subtype" class="inputbox" onchange="this.form.submit()">
					<option value=""><?php echo JText::_('COM_DJCLASSIFIEDS_SELECT_SUBTYPE');?></option>
					<?php echo JHtml::_('select.options', $this->ad_payment_subtypes, 'value', 'text', $this->state->get('filter.subtype'), true);?>
				</select>
			</div>
			<?php } ?>
			<div class="btn-group pull-right hidden-phone">
        ...
        <tr>
            ...
            <td >
                <?php   if($payment->type==5){					 		
								...
								foreach($type_det as $pay_type){
									foreach($a_pay_types as $a_type => $a_type_label){
										if(strpos($pay_type, $a_type) !== false){
											echo '<span style="color:#666;font-size:11px">'.$a_type_label.'</span><br>';
										}
									}
								}
							 }
	                  ...
	                  ?>
			<?php
				$plugin_results = JFactory::getApplication()->triggerEvent('onDJClassifiedsAdminDisplayPaymentsType', array($payment));			
				$plugin_results = trim(implode("\n", $plugin_results));
				echo $plugin_results;
			?>
			</td>
			<td >
            ...
            <?php if($payment->method=='points'){  
                ...
                <select style="width: auto;" name="change_status_<?php echo $payment->id; ?>" class="inputbox" autocomplete="off">			
...
<script type="text/javascript">
    ...
    {
        if (task == 'payments.delete')
        {
            if (confirm('<?php echo DJClassifiedsTheme::sanitizePopupText(JText::_('COM_DJCLASSIFIEDS_DELETE_PAYMENT_CONFIRM')); ?>')) {                
