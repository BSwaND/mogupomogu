						...
						<?php 				
							foreach($blog_sort_v as $sort_v){
								...
								echo '<option value="'.$sort_v.'-asc" '.$option_selected.' >';
									...
									}else if($sort_v=='date_e'){ echo JText::_('COM_DJCLASSIFIEDS_DATE_EXPIRATION');
				...
				if($par->get('blog_desc_position','right')=='right'){
					...
					foreach($i->fields as $f_id => $field){
						if($this->custom_fields[$f_id]->in_blog && $field!=''){
							echo '<div class="cf_box '.$this->custom_fields[$f_id]->name.'">';
								...
								if($this->custom_fields[$f_id]->type=='checkbox'){
									...
									if($par->get('cf_values_to_labels','0') && !in_array($this->custom_fields[$f_id]->type, array('inputbox', 'textarea', 'date_from_to'))){
